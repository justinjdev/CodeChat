...
class Audio extends Component {
        ...
        if (mic) { //inactive if user does not allow access to microphone. Show Alert
            if (vcstate) { //button only active if user is in VC
                if (mediaRecorder.state === "recording") {
                    //if recording, stop recording and clear interval
                    document
                    ...
                    //start recording and set interval
                    document
                        .getElementById("rbutton")
    ...
    //   //preset keyCode for PTT, 90 = 'z'      this check could occur here or
    // before the method is called    if(e.keyCode==kc){        if(vcstate &&
    // mediaRecorder.state != "recording"){   //only active if user is in VC, make
    // sure user isn't already recording(bug)            mediaRecorder.start()
    //  }    } } reckeyup(e) {   //PTT key released        - currently not bound
    // let kc = 90   //preset keyCode for PTT, 90 = 'z'      this check could occur
    // here or before the method is called    if(e.keyCode==kc){        if(vcstate
    // && mediaRecorder.state != "inactive"){   //only active if user is in VC, make
    // sure mediaRecorder isn't already stopped(bug)            mediaRecorder.stop()
    //        }    } }
    vcclick() {
